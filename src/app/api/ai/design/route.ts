...
**重要**: フルテーマ生成時は必ず \`"generate_header_image": true\` と雰囲気に合った \`"header_image_style"\` も返してヘッダー画像も一緒に生成する。
...
**判断基準**: 雰囲気・スタイル全体を示す指示はモード2（＋ヘッダー画像も自動生成）。具体的な要素の変更指示はモード1。画像に言及する指示はモード3（モード1/2と併用可）。
