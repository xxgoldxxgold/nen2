...
export async function POST(request: Request) {
  ...
  if (!ALLOWED_TYPES.includes(file.type)) {
    return NextResponse.json({ error: 'PNG、WebP、SVGのみ対応しています' }, { status: 400 })
  }
  ...
  try {
    ...
    return NextResponse.json({ imageUrl, message: 'ロゴをアップロードしました' })
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error)
    console.error('Upload logo error:', msg, error)
