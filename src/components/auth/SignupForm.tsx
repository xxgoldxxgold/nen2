...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If auto-confirmed (e.g. during development)
    if (data.session) {
  ...
  if (success) {
    return (
      <div className="w-full max-w-md space-y-6 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">メールを確認してください</h2>
        <p className="text-gray-600 dark:text-gray-400">
          <span className="font-medium text-gray-900 dark:text-white">{email}</span> に確認メールを送信しました。
          メール内のリンクをクリックして登録を完了してください。
        </p>
        <Link
          href="/login"
          className="inline-block text-sm font-semibold text-blue-600 hover:text-blue-500 dark:text-blue-400"
        >
          ログインページへ戻る
        </Link>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            className="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-3 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder-gray-500"
