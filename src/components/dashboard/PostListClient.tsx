...
export default function PostListClient({ posts, username }: { posts: Post[]; username: string }) {
  ...
  const statusBadge = (status: string) => {
    ...
    const label = status === 'published' ? '公開' : status === 'scheduled' ? '予約' : '下書き'
  ...
  return (
    <div className="space-y-4">
      ...
      {filteredPosts.length > 0 ? (
        <>
          {/* Desktop table */}
          <div className="hidden overflow-hidden rounded-xl border border-gray-200 bg-white md:block dark:border-gray-700 dark:bg-gray-900">
          ...
          <div className="flex flex-col gap-3 md:hidden">
            {filteredPosts.map((post) => (
              <div key={post.id} className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
                <div className="flex items-start justify-between gap-2">
                  <Link
                    href={`/dashboard/posts/${post.id}/edit`}
                    className="flex-1 text-sm font-medium text-gray-900 hover:text-blue-600 dark:text-white"
                  >
                    {post.title}
                  </Link>
